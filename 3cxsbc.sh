...
# Author: 3CX 
# Editor: 78wesley 
# Editor website: https://78wesley.eu

while getopts u:k:a: flag
do
    case "${flag}" in
        u) URL=${OPTARG};;
        k) KEY=${OPTARG};;
...
function main()
{
	...
	if [ -z "$URL" ];
	then
		echo "-u has not been set (not required)"
		AUTO=0
		else
		echo ""
	fi
	...
	if [ -z "$KEY" ];
	then
		echo "-k has not been set (not required)"
		AUTO=0
		else
		echo ""
	fi
